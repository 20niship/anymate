...
template <typename T> uiVec3<T>::uiVec3(std::string text_, _Vec<T, 3>* value_, Vec2 range_, const double delta_) {
  text                    = text_;
  value                   = value_;
  range                   = range_;
  delta                   = delta_;
...
template <typename T> void uiVec3<T>::render() {
  if(!flags.Active) {
    needRendering(false);
...
template <typename T> bool uiVec3<T>::CallbackFunc(uiCallbackFlags flag, Vec2d vec2_1, [[maybe_unused]] int num_1, [[maybe_unused]] int num_2, [[maybe_unused]] const char** strings) {
...
template uiVec3<double>::uiVec3(std::string, _Vec<double, 3>*, Vec2, const double);
template uiVec3<float>::uiVec3(std::string, _Vec<float, 3>*, Vec2, const double);
template uiVec3<int>::uiVec3(std::string, _Vec<int, 3>*, Vec2, const double);
template uiVec3<unsigned int>::uiVec3(std::string, _Vec<unsigned int, 3>*, Vec2, const double);
