...
template <typename T, std::enable_if_t<std::is_integral_v<T>, std::nullptr_t> = nullptr> std::uint64_t get_digit(const T num) {
...
struct u8cout_ : private std::streambuf, public std::ostream {
  ...
  void flush() {
    ...
    ::WriteConsoleW(::GetStdHandle(STD_OUTPUT_HANDLE), str_utf16.data(), static_cast<int>(str_utf16.size()), nullptr, nullptr);
    oss.str("");
    oss.clear();
#else
    std::cout.flush();
...
inline DWORD enable_escape_sequence() {
  ...
  if(!SetConsoleMode(hOutput, dwMode_orig_ | ENABLE_VIRTUAL_TERMINAL_PROCESSING | DISABLE_NEWLINE_AUTO_RETURN)) {
...
class pbar {
public:
  pbar(std::uint64_t total, const std::string& desc = "") : pbar(total, static_cast<std::uint64_t>(term::get_console_width().value_or(1) - 1), desc){};

  pbar(std::uint64_t total, std::uint64_t ncols, const std::string& desc = "") : total_(total), ncols_(ncols), desc_(desc) {
  ...
  void tick(std::uint64_t delta = 1) {
    ...
    if(!progress_.has_value()) {
      progress_ = 0;
      ncols_    = std::min(static_cast<std::uint64_t>(term::get_console_width().value_or(1) - 1), ncols_);
    }
    ...
    std::uint64_t num_brackets = static_cast<std::uint64_t>(std::round(prog_rate * width_brackets));
    ...
    u8cout_ << std::setw(3) << static_cast<int>(std::round(prog_rate * 100)) << "%" << opening_bracket_char_;
    ...
    if(enable_time_measurement_) {
      ...
      u8cout_ << std::setw(2) << duration_cast<minutes>(dt).count() % 60 << ':' << std::setw(2) << duration_cast<seconds>(dt).count() % 60 << '<';
      if(auto remain_h = duration_cast<hours>(remaining).count(); remain_h > 0) {
        u8cout_ << remain_h % 60 << ':';
      }
      u8cout_ << std::setw(2) << duration_cast<minutes>(remaining).count() % 60 << ':' << std::setw(2) << remaining.count() % 60 << ", " << std::setw(0) << std::fixed << std::setprecision(2) << vel << "it/s]";
    }
    if(progress_ == total_) {
      if(!leave_) {
        u8cout_ << term::clear_line << '\r';
  ...
  void set_description(const std::u8string& desc) { desc_ = reinterpret_cast<const char*>(desc.data()); }
  void set_description(std::u8string&& desc) { desc_ = reinterpret_cast<const char*>(std::move(desc.data())); }
#endif
  void enable_stack() {
    enable_stack_ = true;
    leave_        = false;
  ...
  template <class T> void warn(T&& msg) {
    static_assert(std::is_constructible_v<std::string, T>, "std::string(T) must be constructible");
...
class spinner {
public:
  spinner(std::string text, std::chrono::milliseconds interval = interval_default) : interval_(interval), text_(text), dwMode_orig_(0) {}
  ~spinner() {
